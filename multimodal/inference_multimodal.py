...
BEST_WEIGHT = [
    "cv_models/sigmoid_vcreg_softmax_glu_4423714_b_64_dp_0.1_fold_0_0.1779.bin"]
...
class Multimodal_Soft_GLUMLP_late_concat(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=3):
        ...
        # GLU layers
        self.glu_layers = nn.ModuleList([GLU(glu_dim) for _ in range(num_layers)])
...
for bw in BEST_WEIGHT:
    model = Multimodal_Soft_GLUMLP_late_concat(input_dim=104, hidden_dim=104, output_dim=1)
...
df_test, new_num_cols = feature_engineering(df_test.copy())
...
df_test[train_cols] = df_test[train_cols].fillna(df_test[train_cols].median())

columns_with_nan = df_test[train_cols].isna().any()
...
df_test[num_cols] = (df_test[num_cols] - means) / (stds + EPS)
print(means)
print(stds)
...
df_test["fold"] = -1
for idx, (train_idx, val_idx) in enumerate(gkf.split(df_test, df_test["target"], groups=df_test["patient_id"])):
    df_test.loc[val_idx, "fold"] = idx

df_test = df_test[df_test["fold"] == 0].reset_index(drop=True)
...
test_dataset = ISIC_multimodal_dataset(df_test, TEST_HDF, train_cols, data_transforms["valid"])
...
with torch.no_grad():
    ...
    for step, data in bar:
        ...
        sum_outputs = 0
        for model in models:
            model_output_1, _, _ = model(tabular_data, images)
            model_output_2, _, _ = model(tabular_data, images_2)
            model_output_3, _, _ = model(tabular_data, images_3)
            model_output_4, _, _ = model(tabular_data, images_4)

            model_output_1 = model_output_1.detach().cpu().numpy()
            model_output_2 = model_output_2.detach().cpu().numpy()
            model_output_3 = model_output_3.detach().cpu().numpy()
            model_output_4 = model_output_4.detach().cpu().numpy()

            model_output = (model_output_1 + model_output_2 + model_output_3 + model_output_4) / 4
            sum_outputs += model_output

        sum_outputs /= len(models)
        preds.append(sum_outputs)

preds = np.concatenate(preds).flatten()


df_sub = df_test[['isic_id', 'target']]
df_sub["target"] = preds
...
pAUC = score(df_test[['isic_id', 'target']], df_sub, 'isic_id')
print(pAUC)