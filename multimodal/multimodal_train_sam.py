...
def train(df):
    ...
    for fold in range(N_SPLITS):
        ...
        model = Multimodal_Soft_GLUMLP(hidden_dim=224, output_dim=1).cuda()
        print(
        ...
        PATH = f"Only_tabular_batch_norm_SAM_adaptive_lr_{CONFIG['learning_rate']}_{CONFIG['img_size']}_early_concat_conf_loss_vcreg_just_GELU_{params}_b_{CONFIG['train_batch_size']}_dp_{CONFIG['dropout']}_fold_{fold}"
        ...
        for epoch in range(1, CONFIG['epochs'] + 1):
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                outputs_a, loss_pred_a, z_a, i_a = model(tabular_data, images_1)
                outputs_b, loss_pred_b, z_b, i_b = model(noisy_data, images_2)
                ...
                loss = 75 * bce_loss_batch + 75 * loss_pred + vicreg_batch + vicreg_image
