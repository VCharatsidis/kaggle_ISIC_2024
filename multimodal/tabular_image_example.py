...
def downsample(val_data):
    df_valid_positive = val_data[val_data.target == 1]
    df_valid_negative = val_data[val_data.target == 0]
...
def train(df):
    ...
    for fold in range(folds):
        ...
        model = SimpleEncoder(seq_length, feature_dim, d_model, nhead, num_encoder_layers, dim_feedforward, output_dim, dropout).cuda()
        ...
        PATH = f"models/simple_cnn_params_{params}_b_{CONFIG['train_batch_size']}_dp_{CONFIG['dropout']}_fold_{CONFIG['fold']}.bin"
