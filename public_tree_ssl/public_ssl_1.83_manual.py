...
if construct_preds:
    path = "../multimodal/models/SSL_120_0.15_benign_validated_SAM_adaptive_lr_0.0002_64_vcreg_4162549_b_128_dp_0.1.bin"
    # path = "../multimodal/models/SSL_120_batch_norm_SAM_adaptive_lr_0.0002_64_vcreg_4162549_b_128_dp_0.1.bin"
    print(path)

    model = EfficientNet_pretrained_linear().cuda()
    ...
    test_preds_df.to_csv("ssl_0.15_validated_benign_64.csv", index=False)
    print("preds df:", test_preds_df.shape)
else:
    pred_file = "ssl_60_0.15_validated_benign_64.csv"
    print(pred_file)
...
for pred_col in pred_cols:
    df_train[f'{pred_col}_attri_norm'] = df_train.groupby('attribution')[pred_col].transform(lambda x: (x - x.mean()) / (x.std() + err))

train_res = calculate_normalizations(df_train, columns_to_normalize)
df_train_final = pd.merge(df_train, train_res, on='isic_id', how='inner')
...
if DO_TUNING:
    ...
    study_lgb.optimize(lambda trial: lgb_objective(trial, sampling_ratio, df_train_final, lgb_feats, target_col, group_col), n_trials=200)
    end_time = time.time()
    ...
    study_cb.optimize(lambda trial: cb_objective(trial, seed, sampling_ratio, df_train_final, cat_feats, target_col, group_col, [x for x in new_cat_cols if x in cat_feats]), n_trials=200)
    end_time = time.time()
    ...
    study_xgb.optimize(lambda trial: xgb_objective(trial, seed, sampling_ratio, df_train_final, xgb_feats, target_col, group_col), n_trials=200)
    end_time = time.time()
...
if DO_FEATURE_IMPORTANCE_MODELS:
    lgb_done = len(lgb_feats) <= 120
    xgb_done = len(xgb_feats) <= 80
    cat_done = len(cat_feats) <= 80
    ...
    while not all_done:
        ...
        zero_importance_lgb = feature_importances_lgb[feature_importances_lgb['importance'] <= lgb]['feature'].to_list()
        ...
        zero_importance_cat = feature_importances_cat[feature_importances_cat['importance'] <= 0.0]['feature'].to_list()
        ...
        zero_importance_xgb = feature_importances_xgb[feature_importances_xgb['importance'] <= 0.0]['feature'].to_list()
